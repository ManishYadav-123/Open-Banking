...
def accountInfo(request):
    ...
    if request.method == "GET":
        if "apiKey" in request.GET and request.GET.get("apiKey") in [_.apiKey for _ in Bank3Users.objects.all()]:
            u = Bank3Users.objects.get(apiKey=request.GET["apiKey"])
            res["status"] = 200
            res["message"] = "SUCCESS"
            res["response"] = {
        ...
        if all(list(map(lambda x: x in request.POST, ["apiKey", "emailID", "address"]))) and request.POST.get(
            "apiKey"
        ) in [_.apiKey for _ in Bank3Users.objects.all()]:
            u = Bank3Users.objects.get(apiKey=request.POST["apiKey"])
            res["status"] = 200
            res["message"] = "SUCCESS"
...
def accTransaction(request):
    ...
    if request.method == "GET":
        if "apiKey" in request.GET and request.GET.get("apiKey") in [_.apiKey for _ in Bank3Users.objects.all()]:
            u = Bank3Users.objects.get(apiKey=request.GET["apiKey"])
            trans = Bank3Transactions.objects.filter(Q(senderID=u.accountID) | Q(receiverID=u.accountID)).order_by(
                "-time"
            )
            res["status"] = 200
            res["message"] = "SUCCESS"
        ...
        ) and request.POST.get("apiKey") in [_.apiKey for _ in Bank3Users.objects.all()]:
            u = Bank3Users.objects.get(apiKey=request.POST["apiKey"])
    ...
    if request.method == "GET":
        if "apiKey" in request.GET and request.GET.get("apiKey") in [_.apiKey for _ in Bank3Users.objects.all()]:
            ...
            res["response"] = [
                ...
                for _ in Bank3LoanOffers.objects.filter(accountID=u.accountID)
            ]
            res["status"] = 200
            res["message"] = "SUCCESS"
        else:
